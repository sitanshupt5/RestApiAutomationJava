...
public class CommonThenTestSteps {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CommonThenTestSteps.class);
    ...
    public void iVerifyFieldsInResponse(DataTable table) {
        table.asMap(String.class, String.class).entrySet().stream().skip(1).forEach((Map.Entry<Object, Object> entry) -> {
            httpServiceAssertion.bodyContainsPropertyWithValue((String) entry.getKey(), JsonUtil.getNodeValue(testManagerContext.getScenarioContext().getContext(ApiContext.REQUEST_BODY).toString(), (String) entry.getValue()));
        });
    }
    ...
    public void iVerifyResponse(String customer) {
        ...
        map.forEach((String key, Object value) -> {
            httpServiceAssertion.bodyContainsPropertyWithValue(key, JsonUtil.getNodeValue(request, (String) value));
        });
    }

    @And("I verify attribute values match {string} in Response")
    public void iVerifyAttributeValuesMatchResponse(String customer) throws IOException, URISyntaxException {
        JSONObject validationData = apiUtilManager.readDataFile(testManagerContext, customer, (String) testManagerContext.getScenarioContext().getContext(ApiContext.API_NAME));
        Map<String, Object> map = apiUtilManager.getSchema(customer, (String) testManagerContext.getScenarioContext().getContext(ApiContext.API_NAME));
        map.forEach((String key, Object value) -> {
            if (key.toLowerCase().contains("contains")) {
                httpServiceAssertion.propertyContainsValue((String) value, validationData.get(key));

            } else if (key.toLowerCase().contains("regex")) {
                httpServiceAssertion.propertyMatchesRegexValue((String) value, validationData.get(key));

            } else {
                httpServiceAssertion.bodyContainsPropertyWithExpectedValue((String) value, validationData.get(key));
            }
        });
    }
    ...
    public void i_verify_attributes_in_response(String validationData, String api, DataTable dataTable) throws IOException, URISyntaxException {
        ...
        if (!Strings.isNullOrEmpty(validationData)) {
            JSONObject expectedResults = apiUtilManager.readDataFile(testManagerContext, validationData, api);
            for (Map.Entry<String, String> attribute : attributes.entrySet()) {
                if (!expectedResults.get(attribute.getKey()).equals(null)) {
                    ...
                    httpServiceAssertion.bodyContainsPropertyWithValue(attribute.getValue(), expectedResult);

                } else {
                    httpServiceAssertion.bodyContainsPropertyWithValue(attribute.getValue(), null);
                }
            }
        }
    }
    ...
    public void iSaveTheAccessToken() {
        String response = testManagerContext.getScenarioContext().getContext(ApiContext.RESPONSE_BODY).toString();
        String accessToken = JsonPath.read(response, "$.access_token").toString();
