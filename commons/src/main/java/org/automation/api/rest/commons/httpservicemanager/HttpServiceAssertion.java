...
public class HttpServiceAssertion {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HttpServiceAssertion.class);
    ...
    public void statusCodeIs(final int expectedStatusCode) {
        ...
        logger.info("Expected HTTP status code: {} matches the actual response status code: {}", expectedStatusCode, actualStatusCode);
    }
    ...
    public void bodyContainsPropertyWithValue(String propertyPath, String expectedPropertyValue) {
        ...
        logger.info("Response field value for {} is equal to {}", propertyPath, expectedPropertyValue);
    }
    ...
    public void bodyContainsPropertyWithExpectedValue(String propertyPath, Object expectedPropertyValue) {
        ...
        if ((com.jayway.jsonpath.JsonPath.read(actualResponse, "$." + propertyPath) != null)) {
            if (!com.jayway.jsonpath.JsonPath.read(actualResponse, "$." + propertyPath).toString().equals("[null]")) {
                ...
                if (propertyValueFromResponse.equals("\\[.*") || propertyValueFromResponse.contains("\\\"")) {
                    propertyValueFromResponse = propertyValueFromResponse.replace("[", "").replace("]", "").replace("\\", "");
                }
            }
        }
        if (expectedPropertyValue == JSONObject.NULL) {
            ...
            logger.info("Response field value for {} is : {}." + "\nExpected property value for the property path is: {}." + "\nHence, expected property value matches the actual property value.", propertyPath, propertyValueFromResponse, expectedPropertyValue);
            ...
            String assertionReason = String.format("Response field '%s' value is not equal to '%s' value.", propertyPath, expectedPropertyValue);
            assertThat(assertionReason, propertyValueFromResponse, is(equalTo(expectedPropertyValue.toString())));
            logger.info("Response field value for {} is : {}." + "\nExpected property value for the property path is: {}." + "\nHence, expected property value matches the actual property value.", propertyPath, propertyValueFromResponse, expectedPropertyValue);
        }
    }

    public void bodyDoesNotContainPath(String path) {
        Object responsePath = httpResponseManager.getResponse().jsonPath().get(path);
    ...
    public void propertyContainsValue(String propertyPath, Object containedValue) {
        String actualResponse = httpResponseManager.getResponse().asString();
        String propertyValueFromResponse = null;
        if (!(com.jayway.jsonpath.JsonPath.read(actualResponse, "$." + propertyPath) == null)) {
            if (!com.jayway.jsonpath.JsonPath.read(actualResponse, "$." + propertyPath).toString().contains("[null]")) {
                ...
                if (obj instanceof Double) {
                    ...
                    propertyValueFromResponse = com.jayway.jsonpath.JsonPath.read(actualResponse, "$." + propertyPath).toString().replace("\\/", "/");
                }
                if (propertyValueFromResponse.equals("[.*") || propertyValueFromResponse.contains("\"")) {
                    propertyValueFromResponse = propertyValueFromResponse.replace("[", "").replace("]", "").replace("\"", "");
                }
        ...
        if (containedValue.toString().contains(",")) {
            ...
            for (String value : valuesArray) {
                ...
                logger.info("Response field value for {} is : {}." + "\nValue expected to be contained in the specific path is: {}." + "\nHence, actual property value contains expected value.", propertyPath, propertyValueFromResponse, value);
            }
        } else {
            ...
            logger.info("Response field value for {} is : {}." + "\nValue expected to be contained in the specific path is: {}." + "\nHence, actual property value contains expected value.", propertyPath, propertyValueFromResponse, containedValue);
        }
    }
    ...
    public void propertyMatchesRegexValue(String propertyPath, Object regex) {
        ...
        if (rawValue instanceof List) {
            ...
            if (regex.toString().contains(",")) {
                ...
                for (int i = 0; i < propertyValueFromResponse.size(); i++) {
                    ...
                    for (int ite = 0; ite < valuesArray.length; ite++) {
                        ...
                        String assertionReason = String.format("Response field entity '%s' does not contain value matching the pattern %s.", propertyPath, valuesArray[ite]);
                        assertThat(assertionReason, validate, matchesPattern(valuesArray[ite]));
                        logger.info("Response field value for {} is : {}." + "\nPattern of value expected to be contained in the specific path is: {}." + "\nHence, actual property contains value matching the expected pattern.", propertyPath, propertyValueFromResponse, valuesArray[ite]);
                    }
                }
            }
            ...
            String assertionReason = String.format("Response field '%s' does not contain value matching the pattern %s", propertyPath, regex.toString());
            assertThat(assertionReason, propertyValue, matchesPattern(regex.toString()));
            logger.info("Response field value for {} is : {}." + "\nPattern of value expected to be contained in the specific path is: {}." + "\nHence, actual property contains value matching the expected pattern.", propertyPath, propertyValue, regex.toString());
        }
    }
}